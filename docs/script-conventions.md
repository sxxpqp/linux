...
跟 `.sh` 首行三件套对齐 — 每个 `.md` 笔记(不含子目录 README)在第一个 `#` 标题之后,紧跟一个 `>` 引用块,带**源链接 + 状态**:

```markdown
# <Title>

> 源: https://github.com/sxxpqp/linux/blob/main/<path-to-this-file>
> 状态: ✅ 生产验证 / 验证过 / 学习笔记
...
**为什么要这个 header**:
- 用户在公网(知乎 / 博客)或别人转发场景看到笔记,能一键回到 git 仓库追溯最新版
- 状态跟所在子目录 README 对齐,读者一眼知道这是不是生产经验
- GitHub blob URL 会渲染 markdown,比 raw 链接友好

**子目录 README 不需要** — README 本身就是入口,不会被独立分享。

**模板**(挑一种):

| 形态 | 适合 |
|---|---|
| `> 源: <github URL>` 单行 | 大多数笔记 |
| `> 源: ... · 状态: ✅ ... · 更新: 2026-06-05` 一行多字段 | 想要时间戳的 |
| 表格 `\| 项 \| 值 \|` | 元信息很多(集群 / 端口 / 镜像版本等) |
