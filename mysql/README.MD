...
```
```
mysqldump -h139.198.123.207 -P33306 -uroot -p --single-transaction --master-data=0 --set-gtid-purged=ON --hex-blob --triggers --routines --events --all-databases > `date +%F`backup.sql
```
### 不建议设置支持跳过
```
