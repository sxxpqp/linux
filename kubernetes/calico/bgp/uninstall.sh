#!/usr/bin/env bash
# 系统: Kubernetes (K8s) — 卸载 Calico BGP 模式
# 下载: https://nexus.ihome.sxxpqp.top:8443/repository/raw-githubusercontent/sxxpqp/linux/refs/heads/main/kubernetes/calico/bgp/uninstall.sh
# 用法: bash uninstall.sh [选项]
#
# 跟 operator/uninstall.sh 一样, 反向删除所有 operator 创建的资源
...
for cr in installation apiserver; do
  kubectl get $cr default >/dev/null 2>&1 && LEFTOVER="$LEFTOVER $cr"
done
for ns in calico-system tigera-operator calico-apiserver; do
  kubectl get ns $ns >/dev/null 2>&1 && LEFTOVER="$LEFTOVER ns/$ns"
done
...
log "==== 完成 ===="
[ "$APPLY" != "true" ] && warn "确认后: bash $0 --apply"
