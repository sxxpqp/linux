...
| `prom no data: {"result":[]}` | PromQL 筛选用的 label 跟 ServiceMonitor 的 relabeling 不匹配。kube-prometheus 把 node 名注入 `instance` 而非 `node`(见 `kubernetes/prometheus/manifests/nodeExporter-serviceMonitor.yaml` 第 16-22 行)。 | 默认已用 `instance`。若你的集群 relabel 到别的 label,设 `PROM_NODE_LABEL=<label名>` 覆盖。验证 PromQL:在 Grafana Explore 跑 `node_cpu_seconds_total{instance="<nodeName>"}`,看能不能命中。 |
