...
新申请 1 个公网 IP,绑到 **新 HAProxy 服务器**,只做"公网入站 → ingress 节点"的反代。HAProxy 是**全代理**(新建到后端的连接,源 IP 是它自己的内网 IP),所以 **ingress 节点的回包走 L2 直达 HAProxy,不经默认网关** —— 现有出口/网关/其他业务**零感知**。
...
```
  ...
  新增(只入站):
    client ──<NEW_PUBLIC_IP>──> [HAProxy 服务器] ─192.168.100.x─> ingress 节点 .11/.12
                                  (mode tcp + send-proxy-v2)
                                  回包: ingress 节点 ─L2 直达─> HAProxy 服务器 (不经任何网关)
```
...
| `<NEW_PUBLIC_IP>` | **本方案新入口**,绑 HAProxy 服务器(单机)或漂在两台间(HA) | 阶段 1–2 | **新申请 1 个** |
...
| **`.41`** | **新 HAProxy 服务器 ①(本方案单机)** | **阶段 1** | eth1 管理 IP |
| **`.42`** | **新 HAProxy 服务器 ②(HA 第二台)** | **阶段 2** | eth1 管理 IP |
| `.43–.49` | 边缘 LB / 反代服务器扩容预留 | — | 避开 `.32` 段 |
...
**策略**:不切旧业务域名的 DNS,而是**新申请一个域名**(或子域)指向 `<NEW_PUBLIC_IP>`。业务想迁过来,就在业务侧把调用域名改成新域名;不想迁的继续用旧域名走旧 nginx,**老业务客户端完全无感**。

| 域名 | 解析到 | 用途 | 何时建 |
|---|---|---|---|
| `<OLD_DOMAIN>`(现有业务域名,可能多个) | `116.211.238.197`(现网 nginx) | 现有业务,**不动** | 现状,保留全程 |
| **`<NEW_DOMAIN>`**(新申请,如 `ingress.<yourcorp>.com`) | `<NEW_PUBLIC_IP>`(新 HAProxy) | **本方案新入口**,业务按粒度迁移 | 阶段 1 |
| `test.<NEW_DOMAIN>`(测试子域) | `<NEW_PUBLIC_IP>` | 灰度自测,验证完不删,留作回归 | 阶段 1 |
| `<NEW_DOMAIN_2>`(阶段 3 双活可选) | 双 A:`<NEW_PUBLIC_IP>` + `<NEW_PUBLIC_IP_2>` | 双台 HAProxy 并行 | 阶段 3 |

**为什么这样比"DNS A 记录切流"好**:
1. **零 TTL 风险**:不改旧 A 记录,不存在"部分客户端 DNS 缓存还指旧 IP"的尴尬期。
2. **回滚 = 业务侧改回旧域名**,DNS / HAProxy / 集群**都不用动**,5 分钟级别回退。
3. **迁移粒度由业务自己控制**:谁先迁谁后迁、灰度比例,都是业务侧自己说了算,运维不背锅。
4. **观测干净**:新域名的流量都进新链路,Prometheus / 日志一眼看清新链路负载,不和旧链路混在一起。
5. **旧域名"被迫"切过来的痛苦没了**:老客户端、老配置、写死域名的脚本——它们继续走旧路,直到自然退役。

> **业务侧迁移动作**:把代码/配置/前端里调用的域名从 `<OLD_DOMAIN>` 改成 `<NEW_DOMAIN>`,重新发布即可。短期两边并存对业务无影响。
...
| ☐ **新域名 `<NEW_DOMAIN>`** + 解析控制权 | 1 个新域名(或子域) | **新申请,不动旧域名 A 记录**;详见 §2.4 域名规划 |
...
| ☐ ingress 节点 `:80 / :443` 物理可达 | 从 HAProxy 服务器 `nc -vz 192.168.100.11 443` | 不通则要排查防火墙/路由 |
| ☐ HAProxy 服务器内网网卡能 ping 通 ingress 节点 | `ping 192.168.100.11` | 同上 |
...
2) 接入方式:公网线接新 HAProxy 服务器 eth0 是独立直连还是与现网 nginx 共用接入交换机
...
8) 内网交换机给 HAProxy 服务器提供端口的速率(必须匹配 §2.3 选的网卡)
```
...
**关键点(决定为什么"不影响现业务")**:
- HAProxy 服务器 eth1 内网 IP 和 ingress 节点 **同二层**,后端回包是 `ingress 节点 → 192.168.100.41`,**走 L2 直达,不经任何网关**。
- HAProxy 服务器**不开 `ip_forward`、不当网关、不做 SNAT** —— 它纯代理,不转发任何过路流量。
...
**完全不动旧业务域名的 A 记录**。准备两条新解析:

```
test.<NEW_DOMAIN>   A   <NEW_PUBLIC_IP>       ; 灰度自测,留作回归
<NEW_DOMAIN>        A   <NEW_PUBLIC_IP>       ; 新业务入口
```

迁移由业务侧"把调用域名从 `<OLD_DOMAIN>` 改成 `<NEW_DOMAIN>`"驱动,运维不动旧 DNS、不背切流锅。出问题 = 业务回退到旧域名(秒级)。

### 4.2 步骤 1:HAProxy 服务器系统初始化

```bash
# 关 SELinux + firewalld(本服务器作为入口反代,iptables 后面单独管控)
setenforce 0
...
> **注意**:**绝对不要** `echo 'net.ipv4.ip_forward=1'`。这台服务器不当网关,开了反而可能引入意外的转发路径。
...
# 2) 新域名 DNS 已配 → 直接打公网 IP + Host 头(等同生产解析,也可直接打域名)
curl -H 'Host: test.<NEW_DOMAIN>' http://<NEW_PUBLIC_IP>/
# 期望:看到 "hello from echo-xxxxx"

# 3) 多打几次,看是否轮询到两个 pod(leastconn 不严格轮询,大致均衡即可)
for i in $(seq 1 10); do curl -s -H 'Host: test.<NEW_DOMAIN>' http://<NEW_PUBLIC_IP>/; done
...
**核心策略**:**不切旧域名的 DNS**,业务侧把调用域名改成 `<NEW_DOMAIN>` 即完成迁移。旧业务不迁的继续用旧域名走旧 nginx,**两边永久并存也 OK**。

**推荐迁移顺序**(按风险从低到高):

```
1. 自家测试业务            → 改用 test.<NEW_DOMAIN>     ← §4.8 已验证
2. 1 个低流量内部业务      → 改用 <NEW_DOMAIN>(或它的子域)    ← 跑 1–3 天
3. 一批中流量业务,分批迁  → 每批观察 1 天                       ← 灰度
4. 高流量 / 核心业务       → 业务侧自决,运维只保证新链路容量    ← 各业务自评迁不迁
5. 旧 nginx 上的 ingress 反代规则 → 等到没有业务调用了再撤      ← 不急
```

> 业务侧改动:代码 / 配置 / 前端里 `<OLD_DOMAIN>` → `<NEW_DOMAIN>`,重新发布即可。短期两边并存对业务无影响(同一 ingress 用 Host 头区分,后端是同一组 pod 也行,不同组也行)。
>
> **多业务共用 ingress 时**:在新建的 Ingress 资源里加上 `<NEW_DOMAIN>` 这个 host(同一组 pod 同时挂在新旧 host 下),业务切过来无需后端改动。

**回滚 = 业务侧改回旧域名**(秒级,DNS / HAProxy / 集群**都不用动**)。

| 出问题层级 | 回滚动作 |
|---|---|
| 单个业务异常 | 业务把调用域名改回 `<OLD_DOMAIN>`,重新发布(秒级) |
| 新 HAProxy 整体异常 | 同上,所有迁过来的业务回退到 `<OLD_DOMAIN>` |
| 兜底:暂时屏蔽新入口 | 把 `<NEW_DOMAIN>` 的 A 记录摘掉(让客户端走 DNS 失败,触发降级)|

**没有 DNS TTL 等待、没有 A 记录切流时间窗,这是本策略的最大好处。**
...
| ⚠️ 2 | HAProxy 服务器开了 `ip_forward` → 意外当网关 | **保持 `ip_forward=0`**,本方案纯代理不转发 |
...
| # | 坑 | 防御措施 |
|---|---|---|
| ⚠️ 13 | active 单台扛全量,带宽/PPS 单机上限 | 容量规划:确认 HAProxy 服务器网卡 + CPU 能撑业务峰值,留 50% headroom |
...
| ⚠️ 20 | 没演练业务回退路径 — 真出事不会回滚 | 上线前找 1 个非核心业务演练:把调用域名 `<NEW_DOMAIN>` 改回 `<OLD_DOMAIN>` 重新发布,观察流量回到旧 nginx |
...
### 7.1 加一台 HAProxy 服务器(nginx-new2, 内网 192.168.100.42)
...
# 端到端
curl -I -H 'Host: <NEW_DOMAIN>' http://<NEW_PUBLIC_IP>/
curl -I http://<NEW_DOMAIN>/    # DNS 配好后可直接打域名
