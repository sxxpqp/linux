...
> 源: https://github.com/sxxpqp/linux/blob/main/kubernetes/etcd/recovery.md
> 状态: 验证过
...
- 用**旧 CA** 跑 `kubeadm init` → 新 apiserver 证书是旧根签的
- worker 的 kubelet 证书也是旧根签的 → 互相认识,不用重新 `join`
- etcd snapshot 恢复 → 所有资源对象回来,等于换了机器但东西一样

### 需要什么

| 项 | 说明 |
|---|---|
| 旧的 `/etc/kubernetes/pki/` 完整备份 | **必需**(尤其 `ca.crt + ca.key`) |
| etcd snapshot | 数据源 |
...
kubeadm 检测到已有 `ca.crt` 就不会重新生成,沿用旧的信任体系(详见 [kubeadm-init-process.md § 二](../kubeadm/kubeadm-init-process.md#二生成证书certificate-authority))。
...
**不需要任何操作。** CA 没变,kubelet 证书还是旧的根签的,几秒钟后自动重新连上 apiserver。如果一直没恢复,重启一下 kubelet 即可:
...
| 关键 | 影响 |
|---|---|
| pki 目录必须**单独备份** | 比 etcd snapshot 还重要 |
| pki **在** | worker 不用动,几秒钟自动恢复 |
| pki **丢** | worker 全部 `kubeadm reset` + 重新 join |
| 重建时 admin.conf 默认走 LB | 注意 [kubeadm-reset.md](../kubeadm/kubeadm-reset.md) 提到的指向风险 |
