...
function create_user(){
    echo "#########################################################################################"
    echo -e "\033[1;31m	   3、Create turing account	\033[0m"
    echo "#########################################################################################"
    read -p "Be sure to create an turing account?[y/n]:"
    case $REPLY in 
    y)
	grep -i 'turing' /etc/passwd
        if [ $? == 0 ];then
	    echo -e "\033[1;31m		An turing account has been created	\033[0m"
        else
	    read -p "Please enter your password:" PASSWD
	    useradd -g root turing;echo "$PASSWD" | passwd --stdin turing  > /dev/null
	    if [ $? == 0 ];then
		echo -e "\033[1;31m	turing account created successfully	    \033[0m"
		grep -i "turing" /etc/sudoers
		if [ $? != 0 ];then
		    chmod u+w /etc/sudoers > /dev/null 
		    sed -i '/^root.*ALL=(ALL).*$/a\turing  ALL=(ALL)       NOPASSWD:ALL' /etc/sudoers > /dev/null
		    if [ $? == 0 ];then
		...
		echo -e "\033[1;31m	    turing account created failed	    \033[0m"
		exit 1 
	    fi
	fi
	;;
...
######################
main
#账号:turing 密码(自己定义):Turing@2022 端口(自己定义):2222